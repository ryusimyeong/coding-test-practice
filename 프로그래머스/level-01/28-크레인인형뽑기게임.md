...
moves에 담긴 숫자는 2차 배열 board의 열의 인덱스+1 이다. 0이면 다음 행으로 넘어가고 0이 아니면 별도의 배열에 해당 숫자를 담고 그 숫자를 0으로 할당한다(board 안에서).

basket의 i번째 요소와 i+1번째 요소가 같다면 둘을 없애고 없앤 만큼 answer에 2를 더해준다.
...
const solution = (board, moves) => {
    ...
    while (moves.length > 0) {
        const target = moves.shift();
        for (let row of board) {
            if (row[target-1] === 0) {
                continue;
            } else {
                basket.push(row[target-1]);
                row[target-1] = 0;
                break;
            }
        }
